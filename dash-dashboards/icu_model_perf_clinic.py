...
train_cols_humanreadable = joblib.load("training_cols_humanreadable_onlyclinic.pkl")

app = dash.Dash()

predictions = pd.read_csv("validationset_predictions_medvitalslabresultsloc_onlyclinic.csv",dtype={"patient_dw_id":object})
...
estimator = joblib.load("model_onlyclinic.pkl") 
valid_x = pd.read_feather("validation_dataset_dashboard_onlyclinic.csv")
valid_mapped = pd.read_feather("validation_dataset_dashboard_mapped_onlyclinic.csv")
...
# merged.reset_index(drop=True).to_feather("tmp_merged_dash_onlyclinic.f")
# print(sdfsf)

merged=pd.read_feather("tmp_merged_dash_onlyclinic.f")
...
def update_graph( threshold, rb_choice):
    ...
    if rb_choice == "FP":
        ...
        tmp = tmp1[((tmp1.prediction_label==1) & (tmp1.label_4hr==0) & (tmp1.label_icunonicu=="non-ICU-72"))]
...
def populate_patient_selector( threshold, rb_choice, pattype):
    ...
    if rb_choice == "FP":
        ...
        tmp = tmp1[((tmp1.prediction_label==1) & (tmp1.label_4hr==0) & (tmp1.label=="non-ICU-72"))]
...
def plot_gantt_chart( patient,threshold):
    ...
    print(preds[["patient_dw_id","discharge_summary","icu_prob"]])
